    ...
    <main class="relative mt-16 z-[10] py-4 flex flex-col justify-center items-center">
        <div class="flex flex-col justify-between items-start w-full">
            ...
            <div id="book-section" class="w-full relative z-[10] flex flex-col-reverse md:grid gap-2 md:grid-cols-4 px-2">
                <div class="md:col-span-3 border-r-[0.45rem] border-[#a1e661]">
                    <div class="flex justify-between items-center">
                        <h1 class="text-black text-5xl font-thin">Recent Upload</h1>
                            ...
                            <div class="flex flex-col w-full h-full justify-end">
                                <div class="h-full w-full flex flex-row justify-between items-end relative z-[11] pl-2" style="background: linear-gradient(to top, rgba(9, 38, 53, 0.9), rgba(9, 38, 53, 0.7), rgba(9, 38, 53, 0.4), rgba(9, 38, 53, 0), rgba(9, 38, 53, 0), rgba(9, 38, 53, 0), rgba(9, 38, 53, 0), rgba(9, 38, 53, 0));">
                                    <div href="" class="bg-white rounded-sm px-0.5 flex gap-1 items-center">
                                        ...
                                        <p class="text-xs"><?= $recent['rate'] == NULL ? '0.0' : $recent['rate'] ?></p>
                                    </div>
                                </div>
                                <div class="w-full h-[6rem] px-2 pb-1 text-white bg-[rgb(9,38,53)]/90 relative z-[9]">
                                    <div class="text-[0.7rem] grid grid-cols-8 h-11 ">
                                        <h1 class="text-sm leading-5 col-span-7 overflow-hidden"><?= (strlen($recent['judul']) > 28) ? substr($recent['judul'], 0, 28) . '..' : $recent['judul'] ?></h1>
                                        <a href="" class="col-span-1 relative left-2">
                                    ...
                                    <div class="text-slate-200/90 text-[0.7rem] leading-3 grid grid-cols-2 h-8 ">
                                        <p idbuku='<?=$recent['id_buku']?>' class="col-span-1 text-start"><?= (strlen($recent['penulis']) > 27) ? substr($recent['penulis'], 0, 27) . '..' : $recent['penulis'] ?></p>
                                        <p class="col-span-1 flex justify-end items-start"><?= date('d M Y', strtotime($recent['tahun_terbit'])) ?></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="md:col-span-1  md:h-auto pr-6">
                    ...
                    <div class="flex flex-row md:flex-col overflow-x-auto md:overflow-x-clip mt-3 gap-2">
                        ...
                        $getTop = mysqli_query($con, 
                            "SELECT
                                tbl_buku.*, ROUND(AVG(rating), 1) AS rate
                            FROM tbl_buku 
                            LEFT JOIN ulasanbuku ON tbl_buku.id_buku = ulasanbuku.id_buku
                            ...
                            <div class="w-[20rem] md:w-full bg-cover bg-no-repeat bg-center bg-gray-100 shadow-lg shadow-slate-300/10 flex flex-row justify-start items-center h-24 rounded-lg " style="background-image: url('../../../public/images/buku/<?=$top['image']?>');">
                                <div class="py-2 px-2  flex bg-slate-950/50 backdrop-blur flex-row justify-start items-center w-full h-full">
                                    ...
                                    <div class="w-full h-full ml-2 flex flex-row">
                                        <div class="w-[93%] h-full text-white">
                                            <div class="h-12 font-medium"><?= (strlen($top['judul']) > 40) ? substr($top['judul'], 0, 40) . '..': $top['judul'] ?></div>
                                        </div>
                                        <div class="w-[7%] h-full">
                                            <svg class="absolute -right-3" width="3rem" height="3rem" viewBox="-3.8 -3.8 27.60 27.60" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                                                fill="#ffffff" stroke="#ffffff"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>bookmark_fill [#1227]</title> <desc>Created with Sketch.</desc> <defs> </defs> <g id="Page-1" stroke-width="1.22" fill="none" fill-rule="evenodd"> <g id="Dribbble-Light-Preview" transform="translate(-265.000000, -2679.000000)" 
